...
private fun PolaroidCameraContent(
    ...
    // 0 = the print is still inside the camera, 1 = fully out of the film slot.
    val ejectProgress = remember { Animatable(0f) }
    val printTilt = remember { Animatable(0f) }
            ...
            if (currentPhotoState == PhotoState.Idle || currentPhotoState == PhotoState.Done) {
                coroutineScope.launch {
                    if (currentPhotoState == PhotoState.Done) {
                        // Draw the previous print back into the camera while the next one is taken.
                        launch { ejectProgress.animateTo(0f, tween(EJECT_AWAY_MS, easing = FastOutSlowInEasing)) }
                    }
                    ...
                    takePhoto(
                        ...
                        onImageCaptured = { bitmap ->
                            ...
                            coroutineScope.launch {
                                ejectProgress.snapTo(0f)
                                devGreyAlpha.snapTo(1f)
                                devTealAlpha.snapTo(1f)

                                printTilt.snapTo((-MAX_TILT_DEG..MAX_TILT_DEG).random().toFloat())
                                ejectProgress.animateTo(1f, tween(EJECT_MS, easing = LinearOutSlowInEasing))
    ...
    val handleRetake: () -> Unit = remember {
        {
            if (currentPhotoState == PhotoState.Done) {
                ...
                coroutineScope.launch {
                    ejectProgress.animateTo(0f, tween(RETAKE_MS, easing = FastOutSlowInEasing))
                    capturedBitmap = null
                }
            }
        }
    ...
    PolaroidCameraBody(
        ...
        ejectProgress = ejectProgress.value,
        printTilt = printTilt.value,
